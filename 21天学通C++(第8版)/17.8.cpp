...
int main()
{
	...
	// Display contants on the screen
	for(size_t count = 0;
			count < intDeque.size();
			++count)
	{
	...
	cout << "The contants of the deque after erasing an element ";
	cout << "from the top and bottom are:" << endl;
	
	// Display contents again: this time using iterators
	// id on older compilers, remove auto and uncomment next line
	// deque<int>::iterator element;
	for(auto element = intDeque.cbegin();
			element != intDeque.cend();
			++element)
	{
